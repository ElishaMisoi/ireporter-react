...
const redfagsUrl = `${process.env.BASE_URL}/redflags/`;
var selectedIncident = {};


class Redfags extends Component {
  state = {
    items: [],
    open: false
  };
  ...
  getAllredflags = () => {
    ...
    fetch(redfagsUrl, config)
      ...
      .then(data => {
        data.body.results.forEach(incident => {
        ...
        this.setState({ items: data.body.results });
  ...
  render() {
    ...
    return (
      <div class="body listView">
        <Modal open={open}>
          <Modal.Header>Redfag</Modal.Header>
          <Modal.Content image>
